...
class NotesService{
  ...
  List<DatabaseNote> _notes = []; //cache List

  DatabaseUser? _user;

  static final NotesService _shared = NotesService._sharedInstance(); // FOLLOWING THREE LINES ARE TO MAKE NOTESSERVICE A SINGLETON !!!
  ...
  Future<DatabaseNote> updateNote({
    ...
    if (updatesCount == 0){
      ...
      _notes.removeWhere((note) => note.id == updatedNote.id); // Remove old note from local cache
  ...
  Future<void> open() async{
    ...
    try {
      ...
      _db = db;
      // create user table
      await db.execute(createUserTable); // executes C code for createUserTable
      // create note table
      await db.execute(createNoteTable);
      await _cacheNotes(); // place notes in cache
...
class DatabaseUser {
  ...
  // DatabaseUser should create an instance of itself from the values of this map
  DatabaseUser.fromRow(Map<String, Object?> map) : id = map[idColumn] as int, email = map[emailColumn] as String;

  @override
  String toString() => 'Person, ID = $id, email = $email';

  @override bool operator ==(covariant DatabaseUser other) => id == other.id; // Compare our id
...
class DatabaseNote{
  ...
  @override bool operator ==(covariant DatabaseNote other) => id == other.id; // Compare our id
